...
def journal_entry_extra_charges(source_name):
    ...
    if not source_name.journal_entry_extra_charges_done:
        if not source_name.extra_charges <= 0:
            ...
            source_name.journal_entry_extra_charges_done = 1
            source_name.save()
        else:
...
def journal_entry_detention(source_name):
    source_name = frappe.get_doc("Sales Invoice", source_name)
    if not source_name.journal_entry_detention_done:
        if not int(source_name.detention) <= 0:
            # master data-----------------
            voucher_type = "Journal Entry"
            posting_date = source_name.posting_date
            vehicle_no = source_name.vehicle_no
            ...
            source_name.journal_entry_detention_done = 1
            source_name.save()
        else:
            frappe.throw("Detention can not be 0 or less")
    else:
        frappe.throw("Journal Entry For Detention already created")